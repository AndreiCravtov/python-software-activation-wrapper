...
* Tkinter: this solution was divided into *Server* and *Client*. The latter also has a front end. I chose to make this front end with the Python GUI library `Tkinter`. I learnt to make a responsive, interactive and intuitive interface, where the entry meant for the software activation key only lets the user type allowed characters, and only until the right length. A further demonstration of the focus on interactivity is that the submit button is disabled unless the correct length key is entered. 
