import hashlib
import rsa
import acc # my own cypher algorithm
...
SYMMETRIC_KEY = PUBLIC_KEY._save_pkcs1_der().decode(ENCODING_STANDARD)
...
class CryptoUtils:
    ...
    @staticmethod
    def rsa_decrypt(ciphertext, private_key, encoding_standard=ENCODING_STANDARD):
    ...
    @staticmethod
    def rsa_sign(message, private_key, encoding_standard=ENCODING_STANDARD):
    ...
    @staticmethod
    def acc_encrypt(plaintext, key=SYMMETRIC_KEY):
        """Returns encrypted cyphertext given a key"""

        return acc.encrypt(plaintext, key)

    @staticmethod
    def acc_decrypt(ciphertext, key=SYMMETRIC_KEY):
        """Returns decrypted plaintext given a key"""

        return_value = acc.decrypt(ciphertext, key)

        if return_value:
            return return_value
        else:
            raise Exception("Wrong key error...")
